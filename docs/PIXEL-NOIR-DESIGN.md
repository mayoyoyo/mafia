...
> **USER AMENDMENT 2026-06-12 — readability/affordance carve-out (overrides the stepped-corner mandate where they conflict).** Interactive affordances and text readability beat the pixel treatment. Form controls keep/revert to STOCK pre-D1 styling: toggle switches (pill track + round knob), +/- steppers (`#mafia-minus`/`#mafia-plus` `.btn-counter`), the settings-modal close X (`#btn-close-settings`). Reviewers: these are NOT "leftover rounded corners" — do not flag, do not re-step them. The narrator-voice control is replaced outright (the stock select is ALSO rejected — its long `label — description` option strings bleed out of the panel): a custom expandable picker — collapsed row shows the current voice label; tapping expands to ◀ ▶ arrows (44px tap targets) that cycle the accents, label prominent with the description wrapped below; each step applies immediately via the same `update_settings {narrationAccent}` wire call, and the server-echo path (updateSettingsUI → value sync + preloadNarrationAudio) stays the only state writer. The custom picker MAY be noir-styled (arrows are unambiguous affordances). Text inputs and buttons stay noir. General principle for the rest of the program: if a restyle makes any control less recognizable or any text less readable, stock/readable wins.
>
> **Extension 2026-06-12 (post-D3):** the settings gear buttons (`#btn-settings-lobby-admin`, `#btn-settings-lobby-player`, `#btn-settings`) are ALSO stock: plain pre-D1 button chrome with the original `&#9881;` cog glyph. They are EXEMPT from §5 emoji extinction — reviewers and the D9 final sweep must NOT flag the surviving gear entity or re-pixel it. (GEAR_ART stays in the registry, unused at these sites.)
>
> **Extension 2026-06-12 (post-D5, UX refinement pass):** ALL modal close-X buttons are stock, not just settings — `#btn-close-transcript` joins `#btn-close-settings` in the shared `.btn-icon` neutralization rule (any future modal X too). The **peel-to-reveal hint** (`#peel-hint` element + `.peel-hint`/`peel-hint-pulse`/`.peel-arrow-icon` CSS + its show/hide/icon JS) is **REMOVED ENTIRELY** (user decision — players know the squeeze gesture); do NOT reintroduce it, and this MOOTS the D3/D7 peel-arrow-direction carry-overs.

## §4 Phase ambience

> **USER AMENDMENT 2026-06-12 (post-D5) — phase ambience is OPT-IN via a three-way theme preference.** D2 forced the room dark at night / light at day with no escape, which flattened the manual light/dark toggle. Replace the old 2-state dark toggle (`#toggle-dark-mode`, localStorage `darkMode`) with a **three-way control: Dark / Light / Dynamic (default Dynamic)**, persisted as a client-local pref (migrate old `darkMode`: true→Dark, false→Light, absent→Dynamic).
> - **Dynamic**: the §4 behavior below — phase drives the effective base theme (night/voting→dark, day→light) AND applies the token remaps; the "every phone goes midnight together" effect lives here only.
> - **Dark**: effective base theme stays **dark always** — never flips to light at day. Night/voting may still *flavor* within dark (navy wash, blood tint), but `data-theme` never becomes light.
> - **Light**: effective base theme stays **light/paper always** — the navy night takeover must NOT fire; never flips to dark/navy.
> Implementation contract: `data-theme` is the effective base (dark|light) and is always set; in Dynamic it's computed from phase, in Dark/Light it's pinned. The aggressive light-night navy override must be gated so forced-Light stays light. `theme-color` meta mirrors the effective base. The control replaces the toggle in the settings modal.
>
> **Header polish (same amendment):** the in-game header (`.game-header` → `.phase-indicator` + `.round-indicator` + `.day-timer` + `#btn-settings`) must read CLEAN at night — kill the clumsy navy slab behind the phase pill / round — and the round indicator must be **visually centered** (pill left, round centered, gear right; the round currently drifts left as an unbalanced flex item).
...
> **USER AMENDMENT 2026-06-12 (post-D5) — §6 IS WITHDRAWN. All player avatars are REMOVED.** D4 shipped them and the user rejected them outright on every surface (decorative pre-reveal portraits read as noise; the reveal portraits go too). Revert D4 entirely: no avatars in lobby/status/vote lists, no true portraits at game-over reveal — lists are plain name + color-dot, reveal shows role name + card as before. Remove `avatarIndexFor`/`getCosmeticAvatar` from `pixel-art.js` and their registry tests (keep the D3 grid tests). KEEP the pre-existing `getRoleImage` role-card usage (predates D4, not an avatar). **D6 must NOT add reveal portraits.** The original §6 text below is retained for history only.
...
> **USER AMENDMENT 2026-06-13 (post-D7) — full-card reveal.** D7's diagonal corner-pivot fold tops out at exactly 50% of the card (hard geometric ceiling of any reflect-corner-across-a-crease model — confirmed by research). The user wants the drag to reveal the WHOLE card. D7.5 switches to a **translating straight crease** driven by one normalized progress `t∈[0,1]`: `t→0.5` reproduces D7's corner peek, and past the anti-diagonal the crease endpoints walk onto the top + left edges so the fold sweeps to the opposite (top-left) corner = 100% revealed; release still snaps shut (reverse sweep, secret hidden the instant `t` < the peek threshold). Gesture/secrecy/60px grab zone/quick-peek/card-dark-in-light all preserved. Polygon math, gradient underside + `drop-shadow`, gesture→`t` mapping, and the cap-at-`t≈0.9`-plus-crossfade fallback are in `docs/research/peel-full-card.md`. (The fold runs in 100×100 percent space → crease angle skews on the non-square card; acceptable for a full reveal — don't chase pixel-space exactness unless it reads wrong.)
