...
| C2 | Engine trigger + revenge | b6f2dd4, 384f9d0, fa9c600 (C2a) | C2a DONE (mechanism): observe-and-queue via module-level WeakMap queuedHunterTrigger (survives reset boundary — trap test fails naive design by construction, falsifiability probe re-verified by spec review); consume at concludeRound top w/ E11/E12 suppression; submitHunterRevenge returns {ok, deaths: Death[], messages} (deaths[] deviation from §3.5 sketch ACCEPTED — cause-keyed cascade broadcast per §3.6); §4 invariant relaxation; 5 forced-transition clears pinned; hunter_gate slog family; pendingMessages confirmed §3.2 dead code (comment, not fix). 418 tests, goldens untouched. C3 MUST: re-check game.pendingRevenge after submitHunterRevenge (POST-CONDITION doc'd — cascade can re-open gate); reject cast_vote/cancel_vote while gated (engine-reachable holes, spec-review flagged). C2b (edge matrix E1–E5, E7) next. |
