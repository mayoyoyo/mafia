...
| C2 | Engine trigger + revenge | b6f2dd4, 384f9d0, fa9c600 (C2a); 2ebb32a, d712fe8, 7fe6495 (C2b) | DONE. C2a mechanism: observe-and-queue WeakMap queuedHunterTrigger (survives reset boundary — trap test fails naive design by construction, falsifiability re-probed by spec review); consume at concludeRound top w/ E11/E12 suppression; submitHunterRevenge returns {ok, deaths: Death[], messages} (deaths[] deviation ACCEPTED — cause-keyed cascade per §3.6); §4 invariant relaxation; 5 forced-transition clears pinned; hunter_gate slog family; pendingMessages confirmed §3.2 dead code. C2b edge matrix: tests/hunter-edge-matrix.test.ts (12 tests, E1–E5+E7 engine halves, all liveness-proven via engine sabotages); 1 real bug fixed failing-first — assertInvariants rejected preserved jokerHauntVoters behind the E4-variant gate (narrowest allowance + both directions pinned). 430 tests, typecheck 0, goldens 11/11 untouched. C3 MUST: re-check game.pendingRevenge after submitHunterRevenge (POST-CONDITION — cascade can re-open gate); reject cast_vote/cancel_vote while gated (engine-reachable holes); resume payloads confirmed: dawn {autoNight:false}, vote {autoNight:true}, official-joker {autoNight:true,preserveHauntVoters:true}; sequence broadcasts from game state after the call, don't re-derive narration. C8: extract makeGame/lockTarget/runNight/runVote → tests/helpers/engine-fixtures.ts (3rd copy looming; runVote needs an abstention shape). |
