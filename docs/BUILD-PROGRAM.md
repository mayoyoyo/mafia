...
| B3 | P2 death pipeline | 9d3e52f (prep) | prep DONE: golden games 6-8 (doctor save, both lover cascades, plain execution, town+mafia+house-joker wins; band 21600-21999; 332 tests). Kill-block→golden mapping in review record. Ungolden residue: haunt-source cascade + official-joker cascade orderings — B3 main must pin via engine tests. Main rewrite IN FLIGHT. |
...
### REMAINING
B3 → B8, then all of C. B0+B1 fully done at a40cdb7 (314 tests / 0 fail across 20 files, `bun run typecheck` 0 errors, goldens + 10-player gating everything). Next action: B3 (prep dispatch = golden gap-fill, then death-pipeline rewrite).
