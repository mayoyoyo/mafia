...
| D1 | Fonts wired + tokens + chrome + texture | D1a: 85f749c, D1b: (pending commit) | D1a DONE 2026-06-12: @font-face (6 files), Libre Franklin body stack, Silkscreen on display/buttons/labels, IBM Plex Mono on room-code/timers/copyright/lock-chips; full token swap (--bg #0b0b10, --bg-card #1a1a2e, --bg-elevated/input #15152a, --text #ece5d8, --text-secondary #8a8a96, --border #34345c, --primary #e8a33d, --primary-dark #b87d24, --danger #b3202a); NEW --danger-bright #d4313c, --role-joker #26c6da (cyan, fixes doctor collision), --warning #ffc107 (gold, hue-separated from hunter #ef6c00 AND amber #e8a33d); #ff9800 execution events → --warning; light-theme amber primary + contrast-correct warning; sw precache → mafia-v4 + 6 font paths; preload hints in index.html. 510/510 tests, typecheck 0. D1b DONE 2026-06-12: .pxc (12px/4px standard) + .pxc-sm (6px/2px small chip) + .pxb wrapper; .btn border-radius→0 + pxc clip-path; .btn-primary amber+pressed-shadow; .btn-secondary stepped border via .btn-pxb wrapper in HTML; .input-pxb input wrapper; panels/cards/modals/chips all gain pxc stepped corners (settings-panel, narrator-area, action-panel, voting-panel, admin-controls, mafia-target-card, target-list, event-history, vote-status, role-reveal-item, etc.); .scanlines::after + .vignette::before + .dither-h texture utilities; scanlines applied to all .screen.active + dead/suspense/joker-win overlays; vignette applied to auth/game/gameover screens and overlays; HTML input wrappers + .btn-pxb wrappers on secondary buttons in index.html. D1c (slide reskin) still TODO. 510/510 tests, typecheck 0, screenshots: dark+light both render stepped corners. |
| D2 | Phase ambience | | |
