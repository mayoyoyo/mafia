...
| D5.5a | UX refinement: revert avatars + remove peel hint + transcript X stock (user feedback) | | QUEUED (runs before D6). (1) Revert D4 avatars ENTIRELY per spec §6 withdrawal — pixel-art.js avatarIndexFor/getCosmeticAvatar gone + their registry tests, 5 app.js render sites back to plain name+color-dot, app.css avatar classes gone, index.html #vote-target-avatar span gone, game-over reveal back to role name+card (KEEP pre-existing getRoleImage role-card). (2) Remove peel hint entirely (#peel-hint + CSS + JS show/hide/icon). (3) #btn-close-transcript → stock via the shared .btn-icon rule. |
...
| D7 | Card peel | | D3 carry-over: peel-hint arrow lost its ↘ direction (D3 wired an unrotated right-pointing hand + stale comment at the wiring site) — restore diagonal semantics (rotate(45deg) or dedicated arrow grid) and trim the comment while the peel files are open. |
