...
**Kickoff blurb for a fresh orchestrator (paste after C merges):**
> You are the Program D orchestrator in `/Users/hansonkang/Documents/GitHub/mafia-noir` (branch `feat/pixel-noir`). First acts: (1) verify Program C merged — `git log origin/staging --oneline -20` must show the hunter-role merge; (2) `git fetch origin && git rebase origin/staging`; (3) run `bun test` + `bunx tsc --noEmit` and record the baseline; (4) read `docs/PIXEL-NOIR-DESIGN.md` then `docs/NOIR-PROGRAM.md`, fold C's carry-over notes + hunter surfaces into D1/D3/D4/D5/D6 scope per the gate checklist; (5) execute D1→D9 under the operating protocol (two-stage review, screenshots required, never push).
