...
| D2.5 | Control UX amendments (user feedback 2026-06-12) | | QUEUED. Ground truth = spec §3.3 USER AMENDMENT: (1) revert to STOCK pre-D1 styling (reference: `git show bb5e165:public/app.css`): toggle switches (pill+round knob), mafia +/- stepper (.btn-counter), settings-modal close X (#btn-close-settings); (2) REPLACE `<select id="lobby-accent">` with a custom expandable picker (collapsed = current voice label; tap → ◀ ▶ arrows cycle accents, label + wrapped description, 44px targets, immediate update_settings per step; fixes the select bleeding out of the panel). Standing rule: affordance/readability beats pixel treatment — reviewers must NOT flag the stock controls as leftover rounded corners. |
